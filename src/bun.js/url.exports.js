...
const J = ["/", "?", "#"];
const ot = 255;
const K = /^[+a-z0-9A-Z_-]{0,63}$/;
const at = /^([+a-z0-9A-Z_-]{0,63})(.*)$/;
const nt = { javascript: !0, "javascript:": !0 };
const N = { javascript: !0, "javascript:": !0 };
const R = {
  http: !0,
  https: !0,
  ftp: !0,
...
(m.prototype.resolveObject = function (s) {
  ...
  if (s.slashes && !s.protocol) {
    for (let f = Object.keys(s), h = 0; h < f.length; h++) {
      const g = f[h];
      g !== "protocol" && (t[g] = s[g]);
    }
    return (
  ...
  if (s.protocol && s.protocol !== t.protocol) {
    if (!R[s.protocol]) {
      for (let c = Object.keys(s), v = 0; v < c.length; v++) {
        const j = c[v];
        t[j] = s[j];
      }
  ...
  const b = t.pathname && t.pathname.charAt(0) === "/";
  const P = s.host || (s.pathname && s.pathname.charAt(0) === "/");
  let p = P || b || (t.host && s.pathname);
  const C = p;
  let e = (t.pathname && t.pathname.split("/")) || [];
  var i = (s.pathname && s.pathname.split("/")) || [];
  const d = t.protocol && !R[t.protocol];
  if (
    (d &&
      ((t.hostname = ""),
      (t.port = null),
